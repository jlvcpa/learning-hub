    ...
    return html`
        <div className="border rounded bg-white flex flex-col h-full shadow-sm">
            ...
            <div className="p-4 overflow-y-auto flex-1 text-xs">
                ...
                <div className="mb-4"><div className="font-bold mb-1">Non-Operating Income and Expenses</div><table className="w-full"><tbody>${nonOpItems.map((r,i)=>html`<tr key=${i}><td className="pl-4"><input type="text" className="w-full bg-transparent" value=${r.label} onChange=${(e)=>handleArrChange('nonOpItems',i,'label',e.target.value)} disabled=${isReadOnly}/></td><td className="w-24"><input type="text" className="w-full text-right bg-transparent border-b" value=${r.amount} onChange=${(e)=>handleArrChange('nonOpItems',i,'amount',e.target.value)} disabled=${isReadOnly}/></td><td><button onClick=${()=>deleteRow('nonOpItems',i)}><${Trash2} size=${12}/></button></td></tr>`)}</tbody></table><button onClick=${()=>addRow('nonOpItems')} class="text-blue-600"><${Plus} size=${12}/> Add</button><div class="flex justify-between font-bold"><span>Net Non-Op Income</span><input type="text" class="w-24 text-right" value=${data?.netNonOpIncome} onChange=${(e)=>updateData({netNonOpIncome:e.target.value})} disabled=${isReadOnly}/></div></div>
...
// --------------------------------------------------------
// MERCHANDISING PERIODIC (Detailed COGS)
// --------------------------------------------------------
...
const MerchPeriodicIS = ({ data, onChange, isReadOnly, showFeedback, calculatedTotals, type = "Single" }) => {
    ...
    const expSales = Math.abs(getBal('Sales')); const expSalesDisc = getBal('Sales Discounts'); const expSalesRet = getBal('Sales Returns and Allowances'); const expNetSales = expSales - expSalesDisc - expSalesRet;
    const expPurch = getBal('Purchases'); const expPurchDisc = Math.abs(getBal('Purchase Discounts')); const expPurchRet = Math.abs(getBal('Purchase Returns and Allowances')); const expNetPurch = expPurch - expPurchDisc - expPurchRet;
    const expFreightIn = getBal('Freight In'); const expCostPurch = expNetPurch + expFreightIn;
    const expBegInv = getBal('Merchandise Inventory'); const expTGAS = expBegInv + expCostPurch;
    let expEndInv = 0; adjustments.forEach(a => { if (a.drAcc === 'Merchandise Inventory' || a.crAcc === 'Merchandise Inventory') { expEndInv = a.amount; } });
    const expCOGS = expTGAS - expEndInv; const expGross = expNetSales - expCOGS;
    const totalExpenses = calculatedTotals.isDr; const expOpExp = totalExpenses - (expBegInv + expPurch + expFreightIn + expSalesDisc + expSalesRet);
    const expOpIncome = expGross - expOpExp; const expNonOp = 0; const expNI = expOpIncome + expNonOp;
    
    const updateData = (updates) => onChange({ ...data, ...updates });
    const renderRow = (label, valueKey, expected, isDeduction=false, indent='pl-4') => html`<div className="flex justify-between items-center py-1"><span className=${indent}>${label}</span><input type="text" className=${inputClass(showFeedback && !checkField(data?.[valueKey], expected, isDeduction))} value=${data?.[valueKey] || ''} onChange=${(e)=>updateData({ [valueKey]: e.target.value })} disabled=${isReadOnly} placeholder="0.00"/></div>`;
        ...
        <div className="border rounded bg-white flex flex-col h-full shadow-sm">
            ...
            <div className="p-4 overflow-y-auto flex-1 text-xs">
                ...
                ${type === 'Single' ? html`
                    ...
                    <div className="flex justify-between items-center py-1 font-bold mt-1"><span className="pl-4">Total Expenses</span><input type="text" className=${inputClass(showFeedback && !checkField(data?.totalExpenses, expOpExp))} value=${data?.totalExpenses || ''} onChange=${(e)=>updateData({ totalExpenses: e.target.value })} disabled=${isReadOnly}/></div>
                ` : html`
                    <div className="mt-4 font-bold text-gray-800">Operating Expenses</div>
                    <div className="flex justify-between items-center py-1 font-bold mt-1"><span className="pl-4">Total Operating Expenses</span><input type="text" className=${inputClass(showFeedback && !checkField(data?.totalOpExpenses, expOpExp))} value=${data?.totalOpExpenses || ''} onChange=${(e)=>updateData({ totalOpExpenses: e.target.value })} disabled=${isReadOnly}/></div>
...
const MerchPerpetualIS = ({ data, onChange, isReadOnly, showFeedback, calculatedTotals, type = "Single" }) => {
    ...
    // COGS is just the ledger balance in Perpetual
    const expCOGS = getBal('Cost of Goods Sold'); 
    const expGross = expNetSales - expCOGS;

    // Operating Expenses Calculation
    const totalISDebits = calculatedTotals.isDr; 
    // In Perpetual: Net Income = Net Sales - COGS - Expenses.
    // Total Ledger Debits in IS = Sales Contra + COGS + Expenses.
    // Expenses = Total IS Debits - Sales Contra - COGS
