...
const validateRow = (row, t, tIdx, idx) => {
    ...
    if (tIdx === 0 && idx === 0) {
        ...
        if (val === yyyy && isEmptyOthers) return { date: true };
        return { date: false };
    }
    ...
    if ((tIdx === 0 && idx === 1) || (tIdx > 0 && idx === 0)) {
        ...
        let dateValid = false;
        // First transaction allows "Nov 01", subsequent just "01"
        if (tIdx === 0 && idx === 1) dateValid = val === `${mm} ${dd}` || val === `${mm} ${dd0}`;
        else dateValid = val === dd || val === dd0;
        
        // If date is wrong, return specific failure
        if (!dateValid) return { date: false };
    }
    ...
    if (!row.isDescription && row.acc) {
        ...
        // Determine user intent based on where they put the money
        const isDebitEntry = dr > 0;
        const isCreditEntry = cr > 0;

        if (!isDebitEntry && !isCreditEntry) return { acc: null, amt: null };

        // --- DEBIT VALIDATION ---
        if (isDebitEntry) {
            // Rule: No indentation for Debits
            if (row.acc.startsWith(' ')) return { acc: false, amt: false };
            
            // Find matching debit in transaction source
            const match = t.debits.find(item => item.account === row.acc.trim());
            
            if (!match) return { acc: false, amt: false }; // Wrong account
            
            // Account is correct, check amount
            const amtCorrect = Math.abs(match.amount - dr) <= 1;
            return { acc: true, amt: amtCorrect };
        }

        // --- CREDIT VALIDATION ---
        if (isCreditEntry) {
            // Rule: Exactly 3 spaces indentation for Credits
            const threeSpaces = '   ';
            const startsWith3 = row.acc.startsWith(threeSpaces);
            const fourthCharNotSpace = row.acc[3] !== ' '; 
            
            // It must start with 3 spaces, but the part after must be the account name
            if (!startsWith3 || !fourthCharNotSpace) return { acc: false, amt: false };

            const cleanAccName = row.acc.substring(3); // Remove first 3 chars
            
            // Find matching credit in transaction source
            const match = t.credits.find(item => item.account === cleanAccName);

            if (!match) return { acc: false, amt: false }; // Wrong account

            // Account is correct, check amount
            const amtCorrect = Math.abs(match.amount - cr) <= 1;
            return { acc: true, amt: amtCorrect };
        }
    }

    return { date: true }; // Default fallthrough for existing valid date rows
};

// --- INTERNAL COMPONENTS ---

const StatusIcon = ({ status, show }) => {
    if (!show || status === null || status === undefined) return null;
    return status
        ? html`<${Check} size=${14} className="text-green-600" />` 
        : html`<${X} size=${14} className="text-red-600" />`;
};
    ...
    return html`
        <div className=${`flex h-8 items-center border-t border-gray-100 ${isDesc ? 'bg-white text-gray-600' : ''}`}>
            ...
            <div className="w-16 h-full border-r relative group">
                ${!isDesc && html`
                    <input type="text" 
                        className=${`w-full h-full px-1 text-xs outline-none bg-transparent text-right ${showFeedback && valResult?.date === false ? 'bg-red-50' : ''}`} 
                        value=${row.date || ''} 
            ...
            <div className="flex-1 h-full border-r relative">
                ${isDesc 
                    ...
                    : (!isYearRow && html`
                        <input type="text" 
                            className=${`w-full h-full px-2 pr-6 outline-none font-mono text-xs ${showFeedback && valResult?.acc === false ? 'bg-red-50' : ''}`} 
                            value=${row.acc || ''} 
                        ...
                        <div className="absolute right-1 top-1/2 -translate-y-1/2 pointer-events-none">
                            <${StatusIcon} show=${showFeedback && (row.dr > 0 || row.cr > 0)} status=${valResult?.acc} />
                        </div>
                    `)
                }
            </div>
            ...
            <div className="w-24 h-full border-r relative">
                ${!isDesc && !isYearRow && html`
                    <input type="number" 
                        className=${`w-full h-full px-2 pr-6 text-right outline-none bg-transparent ${showFeedback && valResult?.amt === false && row.dr > 0 ? 'bg-red-50' : ''}`} 
                        value=${row.dr||''} 
                    ...
                    ${row.dr > 0 && html`
                        <div className="absolute right-1 top-1/2 -translate-y-1/2 pointer-events-none">
                            <${StatusIcon} show=${showFeedback} status=${valResult?.amt} />
                        </div>
                    `}
                `}
            </div>
            ...
            <div className="w-24 h-full border-r relative">
                ${!isDesc && !isYearRow && html`
                    <input type="number" 
                        className=${`w-full h-full px-2 pr-6 text-right outline-none bg-transparent ${showFeedback && valResult?.amt === false && row.cr > 0 ? 'bg-red-50' : ''}`} 
                        value=${row.cr||''} 
...
export const validateStep02 = (transactions, currentAns) => {
    ...
    transactions.forEach((t, tIdx) => {
        ...
        // Note: We check the specific row responsible for the date
        const dateRowIndex = tIdx === 0 ? 1 : 0; 
        if (rows[dateRowIndex]) {
            const dateRes = validateRow(rows[dateRowIndex], t, tIdx, dateRowIndex);
            if (!dateRes.date) txValid = false;
        } else {
            txValid = false;
        }

        // 2. Validate Debits
        // Every expected debit must exist exactly once with 0 identation
        ...
        // 3. Validate Credits
        // Every expected credit must exist exactly once with 3 space indentation
